...
const ChatPanel: React.FC<ChatPanelProps> = ({ messages, sources, notes, isLoading, isFetchingSources, error, onSendMessage, onAddNote }) => {
  ...
  const getPlaceholderText = () => {
    if (isFetchingSources) return "Đang tải nguồn, vui lòng chờ...";
    if (sources.length === 0) return "Vui lòng thêm nguồn trước";
    return "Đặt câu hỏi dựa trên các nguồn của bạn...";
  }
    ...
    <div className="flex flex-col h-full bg-gray-100">
        ...
        <div className="absolute inset-0 overflow-y-auto p-4 sm:p-6">
            ...
            {isFetchingSources ? (
                <div className="text-center text-slate-600 p-10 neumorph-pressed">
                    <BookIcon className="w-20 h-20 mx-auto text-[#161D6F] animate-pulse mb-4"/>
                    <h2 className="text-2xl font-bold text-[#161D6F]">Đang tải nguồn dữ liệu...</h2>
                    <p className="mt-2 max-w-md mx-auto">Vui lòng chờ trong khi chúng tôi chuẩn bị môi trường làm việc cho bạn.</p>
                </div>
            ...
            {!isFetchingSources && messages.map(msg => (
                ...
                <div className="flex items-start gap-4 flex-row chat-message">
                    ...
                    <div className="px-4 py-3 max-w-lg neumorph-raised animate-pulse">
                        <div className="flex items-center space-x-2 text-slate-600">
                            ...
                            <span className="text-sm font-medium">Đang tổng hợp câu trả lời...</span>
                        </div>
                    </div>
                </div>
            )}

            {/* This local error display is removed to avoid redundancy with the global error banner in App.tsx */}
            {error && !isLoading && (
      ...
      <div className="flex-shrink-0 border-t border-slate-200">
        <div className="p-4">
            ...
            {/* The error message previously here is now handled globally in App.tsx */}
            <form onSubmit={handleSubmit} className="flex items-center gap-3 neumorph-raised p-2">
                ...
                disabled={isLoading || isFetchingSources || sources.length === 0}
                className="flex-grow bg-transparent focus:outline-none neumorph-pressed p-3 disabled:cursor-not-allowed text-sm"
                />
                <button
                type="submit"
                disabled={isLoading || isFetchingSources || !input.trim() || sources.length === 0}
