...
Đây là các nguồn:`;
    ...
    sources.forEach(source => {
        if (source.content.mimeType.startsWith('text/')) {
            ...
            // Tách tiền tố data URL để lấy chuỗi base64 thô
            const base64Data = source.content.data.split(',')[1] || source.content.data;
    ...
    try {
        ...
        console.error("Lỗi khi truy vấn API Gemini:", error);

        let userFriendlyMessage = "Đã xảy ra lỗi không xác định. Vui lòng thử lại sau.";

        if (error instanceof Error) {
            const errorMessage = error.message.toLowerCase();

            if (errorMessage.includes("api key not valid")) {
                userFriendlyMessage = "Khóa API không hợp lệ. Vui lòng liên hệ quản trị viên để kiểm tra cấu hình.";
            } else if (errorMessage.includes("rate limit") || errorMessage.includes("429")) {
                userFriendlyMessage = "Bạn đã gửi quá nhiều yêu cầu. Vui lòng đợi một chút rồi thử lại.";
            } else if (errorMessage.includes("candidate was blocked") || errorMessage.includes("safety settings") || errorMessage.includes("empty_response")) {
                userFriendlyMessage = "AI không thể tạo phản hồi cho câu hỏi này, có thể do các bộ lọc an toàn. Vui lòng thử diễn đạt lại câu hỏi của bạn.";
            } else if (error.name === 'SyntaxError') { // JSON.parse error
                userFriendlyMessage = "AI đã trả về một định dạng không mong muốn. Vui lòng thử lại. Nếu sự cố vẫn tiếp diễn, hãy thử diễn đạt lại câu hỏi của bạn.";
            } else {
                userFriendlyMessage = "Không nhận được phản hồi hợp lệ từ AI. Mô hình có thể đã gặp sự cố khi tạo câu trả lời. Vui lòng thử diễn đạt lại câu hỏi của bạn.";
            }
        }
